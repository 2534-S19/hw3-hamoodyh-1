...
// TODO: Write a function that can initialize a Timer with particular values for pre-scale and count.
...
void initTimer(unsigned int timer, unsigned int preScaler, unsigned int count)
{
    // For the specified timer and pre-scale value, put the timer in 32-bit periodic mode.
       Timer32_initModule(TIMER0, TIMER0_PRESCALER, TIMER32_32BIT, PERIODIC);///////////
...
bool timer1Expired(void)
{
    ...
    static unsigned int before = MAX_VALUE;
    bool flag = false;
    present = Timer32_getValue(TIMER1);
    if (present > before)
    {
        flag = true;
    }
